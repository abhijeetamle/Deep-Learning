...
dropout_h1_model_1 = tf.layers.dropout(h1_model_1, 0.5)
h2_model_1 = tf.layers.dense(dropout_h1_model_1, units=10, activation=tf.nn.relu, name='h2_model_1')
dropout_h2_model_1 = tf.layers.dropout(h2_model_1, 0.5)
h3_model_1 = tf.layers.dense(dropout_h2_model_1, units=5, activation=tf.nn.relu, name='h3_model_1')
...
dropout_h3_model_2 = tf.layers.dropout(h3_model_2, 0.5)
h4_model_2 = tf.layers.dense(dropout_h3_model_2, units=50, activation=tf.nn.relu, name='h4_model_2')
...
optimizer_model_1 = tf.train.GradientDescentOptimizer(learning_rate=0.085)
...
optimizer_model_2 = tf.train.GradientDescentOptimizer(learning_rate=0.055)
...
epoch_arr, loss_arr_1, loss_arr_2, loss_arr_3 = [], [], [], [] 
max_epoch = 1000
...
while not converged_m1:
  ...
  for i in range(200): 
    ...
    # Calculating gradients and hessian for every 10 steps
    if i % 10 == 0:
      ...
      grads_vals, hess_vals = sess.run([grads, hessian], feed_dict={input_m1_x: x, output_m1_y: y})

      # Calculate gradient norm
      grad_norm = calculate_grad_norm(grads_vals)
      grad_arr_m1.append(grad_norm)

      loss_itr_arr.append(l_1)
...
# Gradient norm and loss vs total iterations

fig, (ax1, ax2) = plt.subplots(2, sharex=True)
fig.suptitle('Model 1')
ax1.plot(iteration_arr, grad_arr_m1)
ax2.plot(iteration_arr, loss_itr_arr, 'r')
